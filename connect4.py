...
def draw_board(canvas, board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT-1, -1, -1):  # 從底部開始繪製
...
def play_game1():
    ...
    canvas.pack()
    canvas.grid(row=1, columnspan=COLUMN_COUNT)  # 改為使用 grid 佈局

    for col in range(COLUMN_COUNT):
        button = tk.Button(window, text=f"Column {col+1}", command=lambda c=col: button_click(c),bg="lightskyblue")
        button.grid(row=ROW_COUNT, column=col)  # 改為使用 grid 佈局
    draw_board(canvas, board)
    #window.mainloop()
            ...
            if is_valid_location(board, col):
                ...
                if winning_move(board, piece + 1):
                    ...
                    if piece == 0:
                        ...
                        label = tk.Label(window, text="Player2 Win!!", font=('Arial', 20),justify='center', bg='hotpink',fg='black')
                    label.place(relx=0.5, rely=0.5, anchor='center')
                    #messagebox.showinfo("Game Over", f"Player {piece + 1} wins!")
                    game_over = True
